...
import { webacyTradingLite } from "@/lib/webacy";
...
/**
 * GET /api/dd/trading-lite?mint=<base58>
 * Get trading lite data from Webacy Data Driven API
 */
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const rateLimitResult = rateLimitHelper(clientIP, "dd_trading_lite", 60, 60_000);
    if (!rateLimitResult.allowed) {
      safeLog("warn", "DD trading-lite rate limit exceeded", {
        requestId,
        ip: clientIP,
    ...
    // Call Webacy API
    const result = await webacyTradingLite(sanitizedMint, chain);
    ...
    const isDev = process.env.NODE_ENV === "development" || process.env.VERCEL_ENV === "development";
    safeLog("error", "DD trading-lite error", {
      requestId,
      error: err?.message,
